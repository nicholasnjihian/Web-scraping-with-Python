...
for match in tournament_games:
    ...
    team_names = []
    team_ids = []
    team_names.append(team_info_json['event']['homeTeam']['name'])
    team_ids.append(team_info_json['event']['homeTeam']['id'])
    team_names.append(team_info_json["event"]["awayTeam"]["name"])
    ...
    # Maybe ensure there are at least 4 (previus) home matches for the home team &
    ...
    while attempt < max_retries:
        try:
            ht_prev_matches_json = json.loads(driver.execute_cdp_cmd('Network.getResponseBody',{'requestId': log['params']['requestId']})['body'])['events']
